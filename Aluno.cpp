...
bool Aluno::adicionar(Bolsa* bolsa) {
    ...
    for (int i = 0; i < quantidade; i++) {
        if (temConflitoDeBolsas(bolsa->getInicio(), bolsa->getFim(),
                                 bolsas[i]->getInicio(), bolsas[i]->getFim())) {
            return false;
        }
    }
...
double Aluno::getValor(Data* data) {
    for (int i = 0; i < quantidade; i++) {
        Bolsa* b = bolsas[i];
        bool depoisInicio = (data->getAno() > b->getInicio()->getAno()) ||
                            (data->getAno() == b->getInicio()->getAno() &&
                             data->getMes() >= b->getInicio()->getMes());

        bool antesFim = (data->getAno() < b->getFim()->getAno()) ||
                        (data->getAno() == b->getFim()->getAno() &&
                         data->getMes() <= b->getFim()->getMes());
